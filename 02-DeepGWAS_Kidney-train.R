...
library(pbapply);
library(tensorflow);
use_condaenv("RTensorFlow", required = TRUE)
...
example_data <- read.table("/proj/yunligrp/users/theowu/DeepGWAS/data/Gorski.2017.features.all.selected.txt",header=T)

dim(example_data) # 13602    23
head(example_data)
...
# model dir:
model_dir = "/proj/yunligrp/users/theowu/DeepGWAS/model/"
...
for (v in 1:length(validation_split)){
  for(b in 1:length(batch_size)){
    for(e in 1:length(epoch)){
      ...
      save_model_hdf5(model, file = paste0("DeepGWAS_Kidney_Model.h5"))
